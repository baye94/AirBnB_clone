...
class HBNBCommand(cmd.Cmd):
    ...
    def default(self, line):
        ...
        l1 = line.split('.', 1)
        if len(l1) < 2:
            print('*** Unknown syntax:', l1[0])
            return False
        clsname, line = l1[0], l1[1]
        if clsname not in list(self.clslist.keys()):
            print('*** Unknown syntax: {}.{}'.format(clsname, line))
            return False
        l2 = line.split('(', 1)
        if len(l2) < 2:
            print('*** Unknown syntax: {}.{}'.format(clsname, l2[0]))
            return False
        mthname, args = l2[0], l2[1].rstrip(')')
        ...
        if mthname == 'all':
            ...
            if d and type(d) is dict:
                ...
                l3 = list(shlex(args))
                l3[0] = l3[0].strip('"')
                self.do_update(" ".join([clsname, objid, l3[0], l3[1]]))
